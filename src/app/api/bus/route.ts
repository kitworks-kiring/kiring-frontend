    ...
    try {
      ...
      const results = await Promise.all(
        STATIONS.map(async (station) => {
          ...
          const buses: BusArrivalRaw[] = (data.msgBody.itemList || [])
            .slice()
            // 1. 도착 예정 시간(exps1) 기준으로 오름차순 정렬
            .sort((a: BusArrivalRaw, b: BusArrivalRaw) => {
              const t1 = Number(a.exps1) || 99999 // exps1: 남은 시간(초), 값이 없으면 큰 값으로 처리
              const t2 = Number(b.exps1) || 99999
              return t1 - t2
            })
            // 2. 필요한 필드만 추출해서 새로운 객체로 반환
            .map((bus: BusArrivalRaw) => {
              const filtered: Partial<BusArrivalRaw> = {}
              FIELDS.forEach((key) => {
                filtered[key as keyof BusArrivalRaw] = bus[key as keyof BusArrivalRaw]
              })
              return filtered as BusArrivalRaw
            })
          // buses 배열에서 mkTm(생성시각) 중 가장 최근(최대값) 구하기
          const mkTm =
            buses.length > 0
              ? buses.reduce((max: BusArrivalRaw, cur: BusArrivalRaw) => {
                  return Number(cur.mkTm) > Number(max.mkTm) ? cur : max
                }, buses[0]).mkTm
              : ''
          // 정류장 이름, 최신 mkTm, 가공된 버스 리스트를 반환
